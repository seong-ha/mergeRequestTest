					...
					<div class="card-body">
						...
						<div class="insert">
					        ...
					        <div class="file-list">
					        	<c:forEach items="${files}" var="file" varStatus="status" >
					        		<div id="file${file.files_no}" class="filebox" data-no="${file.files_no}">
							            <p class="name">${file.origin_name}</p>
						            	<a class="delete" onclick="deleteFileCutsom(${file.files_no})">×</a>
						            </div>
					        	</c:forEach>
					        </div>
					        <input id="files" type="file" onchange="addFile(this);" multiple />
						</div>
						...
						<div class="mb-3">
							...
							<c:forEach items='${tags}' var="tag">
								<input type="hidden" id="0" data-tag_no="${tag.tag_no}" class="hiddenTags" value="${tag.tag_name}" disabled>
							</c:forEach>
						</div>
						...
						<button type="button" id="reg" class="btn btn-outline-warning" onclick="submitForm()">수정하기</button>
						<button type="button" class="btn btn-outline-danger" onclick="goBack('${returnUrl}', '${returnUrl2}')">돌아가기</button>
					</div>
				</div>
			</div>
		</form>
...
// 		goBack();	// 돌아가기 이벤트 등록
		...
		$('.hiddenTags').each(function(idx, val) {
			let value = val.value;
			tagify.addTags(value);
			let data = {
				"tag_no" : val.dataset.tag_no,
				"tag_name" : val.value,
				"__tagId" : tagify.value[idx].__tagId
			};
			originTags.push(data);
		});
		
		tagify.on('remove', function(e) {
			console.log(originTags);
			console.log(e.detail.data.__tagId);
			for (let i = 0; i < originTags.length; i++) {
				if (e.detail.data.__tagId == originTags[i].__tagId) {
					deletedTag_nos.push(originTags[i].tag_no);
				}
			}
		})
	...
	function selectMenu(menuType) {
		document.getElementById(menuType).className += ' selected-menu';
		/* $('#${menuType}').addClass('selected-menu');	// 선택한 메뉴 */
	}
	...
	/* 삭제할 파일 리스트 추가 및 화면 목록에서 삭제 */
	function deleteFileCutsom(num) {
	    document.querySelector("#file" + num).remove();
	    deletedArr.push(num);
	    console.log(deletedArr);
	    console.log(typeof deletedArr[0]);
	}
		...
		if (window.confirm("수정하시겠습니까?")) {
		    ...
		    if (deletedArr.length > 0) {
		    	for (let i = 0; i < deletedArr.length; i++) {
		    		formData.append("deletedFiles", deletedArr[i]);
		    	}
		    }
	 ...
	 */
		    ...
		    // tag input의 태그 내용 가져와서 tags hidden input에 넣어주기
			...
			for (let i = 0; i < originTags.length; i++) {
				formData.append("originTagNos", originTags[i].tag_no);
				formData.append("originTagNames", originTags[i].tag_name);
			} 
	...
	function goBack(returnUrl, returnUrl2) {
		location.href = returnUrl2 + '&returnUrl=' + returnUrl;
	}
